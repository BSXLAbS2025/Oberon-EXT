...
void ui_refresh() {
    // Цвет рамок зависит от режима: красный для sudo, циановый для обычного
...
void ui_show_help() {
    ...
    ui_log("search <word>   - Search modules in DB", 1);
...
void ui_add_law(int port, const char* desc) {
    ...
    // Рамка сайдбара с учетом текущего режима
    int frame_color = is_pwn_mode ? 3 : 1;
    wattron(gui.sidebar, COLOR_PAIR(frame_color)); box(gui.sidebar, 0, 0); wattroff(gui.sidebar, COLOR_PAIR(frame_color));
